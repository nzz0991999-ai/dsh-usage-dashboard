	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + CSS_ID + '"]') === null) {
			...
			tag.textContent = [
				...
				".dshud_field_hint{font-size:11px;color:var(--dsw-alias-label-tertiary);line-height:1.45;white-space:pre-line}",
				...
				".dshud_btn_danger{background:transparent;border-color:rgba(239,68,68,0.4);color:#ef4444}",
				...
				".dshud_msg_err{background:rgba(239,68,68,0.1);color:#ef4444}",
		...
		const zh = {
			...
			"settings.token.hint": "获取(约 1 分钟):用 Chrome/Edge 登录 platform.deepseek.com → 按 F12 → Application/应用 → Local Storage/本地存储 → 点击 platform.deepseek.com → 复制 userToken 的 Value(值)粘贴到上方(不带字段名/引号/空格;找不到就刷新页面或重新登录再试)。\n安全:token 只保存在本机宿主端的 0600 权限密钥文件,仅用于 DeepSeek 官方只读查询,不会进入任何对话、上传或第三方;浏览器只显示脱敏值。请勿把 token 粘贴到终端、聊天或 Issue。",
